...
# Strict whitelist of official Indian Government domains
...
def is_verified_government_domain(url: str) -> bool:
    """
    Validates whether the input URL originates strictly from an official
    Government of India (.gov.in or .nic.in) portal.
    """
    if not url:
        return False
    ...
    try:
        ...
        for domain in OFFICIAL_GOVT_DOMAINS:
            if hostname == domain or hostname.endswith("." + domain):
                return True
        
        # General check for .gov.in and .nic.in TLDs
...
def compute_sha256(content: bytes) -> str:
    """Calculates SHA-256 hash of document bytes for official verification."""
    return hashlib.sha256(content).hexdigest()

def extract_rule_comparison(text_content: str) -> Dict[str, str]:
    """
    Extracts Old Rule vs New Rule comparative summary from Gazette text.
    """
    lines = [l.strip() for l in text_content.splitlines() if l.strip()]
    old_rule = "Original statutory provision prior to amendment."
    new_rule = "New amended provision enacted under Gazette notification."
    impact = "Immediate legal applicability across Indian jurisdiction."
    
    for line in lines:
        if "repealed" in line.lower() or "substituted" in line.lower():
            old_rule = line
        elif "inserted" in line.lower() or "shall be read as" in line.lower():
            new_rule = line

    return {
        "summary_old_rule": old_rule,
        "summary_new_rule": new_rule,
        "key_citizen_impact": impact,
    }

INITIAL_VERIFIED_ANNOUNCEMENTS: List[Dict[str, Any]] = [
    {
        "id": 1,
